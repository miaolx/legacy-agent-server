...
1.  **Get PR Details:**
    ...
    *   **Key Outputs:** Extract and retain:
        *   Basic PR information (\`metadata\`), including (\`projectId\`, \`mergeRequestIid\`, \`title\`, \`description\`, \`author\`, \`url\`, \`state\`,\`number\`,\`baseRef\`, \`headRef\`, \`headSha\`, \`associatedIssues\`). associatedIssues is a list of Issue metadata.
...
3.  **Get File Dependency Graph:**
    *   Use the \`getGitlabActionArtifactContent\` tool (or a similar tool) to download and parse the file-level dependency graph JSON data (\`dependencyGraph\`) previously generated by CI/CD.
