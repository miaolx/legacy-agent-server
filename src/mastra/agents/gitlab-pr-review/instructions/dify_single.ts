...
3. ** 生成并发布文件级评论 (条件性) **:
  *  基于对实际\`patch\`的审查结果和影响分析，并结合获取的上下文信息(如果存在)，为当前分组生成具体的、全面的、可操作的**中文**评审评论。
    ...
    *  根据定义的【P1、P2、P3、P4】四档分类， 评估代码变更问题的评审优先级。如果有多个代码变更问题，以最高优先级为准。
    *  **合并前面步骤中获得的所有关键输出：**
      *  \`projectId\` (来自 \`metadata\`)
      *  \`mergeRequestIid\` (来自 \`metadata\`)
      *  \`commit_id\` (**必须使用输入 \`metadata\` 中的 \`headSha\`**)
      *  \`path\` (当前评论针对的文件路径)
      *  \`line\` (**评论在\`patch\`中的目标行号，多个评论时取最后一个评论行号**)
      *  \`text\` (你生成的评论文本，)
      *  \`priority\` (评审优先级)
    *  将这些数据构建成一个 JSON 对象。
